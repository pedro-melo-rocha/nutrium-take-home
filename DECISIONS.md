...
- **`slot_unavailable` is a distinct mail from `rejected`** — overlap auto-reject is a slot conflict, not a personal "no". Status is `rejected` (see Data); the separate mail keeps the copy honest. Mailer action is chosen in code, decoupled from the persisted status.
...
- **Status enum: `pending` / `accepted` / `rejected`** — three states, matching the spec vocabulary. Both guest-supersede and overlap auto-kill fold into `rejected` (spec: overlapping pendings are "automatically rejected"). Email copy still distinguishes the overlap case via the `slot_unavailable` mail.
...
- **Transactional invalidation on new pending submit** — app marks prior pendings `rejected` before insert; unique index is the race-safety net.
...
- **Distance sort = opt-in "near me" geo mode** — valid `lat`+`lng` → order by Haversine distance (raw SQL, no PostGIS dep) from the guest's point, ranked by each nutritionist's nearest coord-bearing service; city filter + Braga default bypassed, `location` returns null, cards carry `distance_km`, `sorted_by: "distance"`. Reference point is the guest (matches the locate-me UI), not the typed city. Online services (no coords) excluded. Partial/invalid coords silently fall back to location mode. Coordinates live on `service` (services are the physical locations).
- **Accept/reject nested under nutritionist** — `PATCH /nutritionists/:nutritionist_id/appointment_requests/:id`; the request is looked up through the nutritionist, so one belonging to a different professional 404s (enforces the spec's "answered by the same nutritionist" even without auth).
